...
The serializer's default link extension is `.html`, which is useful for generated HTML documentation but is not correct for bashlog's committed GitHub Markdown landing page.  Because `doc/adr/README.md` sits beside the source ADR Markdown files, bashlog should request `.md` links explicitly and should not add a prefix.
...
- Keep repository-specific Markdown layout in Make rather than in adrctl.
...
3. a fenced Mermaid block containing raw output from `adrctl generate graph --format mermaid -e .md`;
...
```mermaid
<adrctl generate graph --format mermaid -e .md>
```
...
For the committed ADR landing page, Make SHALL invoke graph generation with:
...
This produces node URLs that resolve to ADR Markdown basenames in the same directory as the generated README.
...
3. Graph node URLs target `.md` ADR files for the default committed GitHub landing page.
4. Mermaid source remains human-readable and reviewable in the committed Markdown file.
5. `make adr-index` and `make docs` remain offline after dependency preparation.
6. The generated README remains atomically replaced only after complete successful composition.
7. The text TOC remains available even when a renderer does not support Mermaid or clickable nodes.
8. No Mermaid rendering runtime is added to bashlog consumer artifacts or documentation dependencies.
...
- repository-specific page layout leaking into adrctl's public responsibility;
...
- the ordinary text TOC MUST remain present as a non-Mermaid navigation path.
...
This could reduce the Make recipe.  It was rejected because the graph is one report fragment inside a repository-specific page.  adrctl should own graph semantics and serialization; bashlog should own document layout.
...
The committed ADR README becomes larger because it now carries readable Mermaid graph source in addition to the TOC.  That is an intentional tradeoff: the graph is useful repository-facing documentation and remains deterministic generated text.
...
```text
adrctl
    -> ADR-specific report content

Make
    -> repository-specific composition and placement
```

The new serializer simply adds another focused adrctl report to that composition.
